...
@group(3) @binding(4) var overlay_texture_2: texture_2d<f32>;
@group(3) @binding(5) var overlay_sampler_2: sampler;
...
fn fragment(in: MeshVertexOutput) -> @location(0) vec4<f32> {
    ...
    let color = mix(color, overlay_color, color.g);

    let overlay_color = textureSample(overlay_texture, overlay_sampler, fract(in.world_position.xy * 0.0069444444));

    return mix(color, overlay_color, color.r);
}
//https://youtu.be/eYlBociPwdw?t=168