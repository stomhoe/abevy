...
use crate::tilemap_positioning::HashablePosVec;

#[derive(Component, Debug, Clone)]
#[relationship_target(relationship = Chunk)]
///Component for regions
pub struct ActiveChunksInRegion(Vec<Entity>);

#[derive(Component, Debug, Clone)]
#[relationship(relationship_target = ActiveChunksInRegion)]
pub struct Chunk {
    #[relationship]
...
pub struct ActivatingChunks(pub Vec<ChunkPos>);

impl ActivatingChunks {
    pub fn with_capacity(act: &LoadChunksAround) -> Self {
        Self(Vec::with_capacity(act.discovery_range as usize * 2 + 1))
    }
    ...
    pub fn insert_positions_around(&mut self, center_chunk_pos: ChunkPos, chunk_range: i32) {
        for y in (center_chunk_pos.y() - chunk_range + 1)..(center_chunk_pos.y() + chunk_range) {
            for x in (center_chunk_pos.x() - chunk_range + 1)..(center_chunk_pos.x() + chunk_range) {
                ...
                if !self.0.contains(&chunk_pos) {
                    self.0.push(chunk_pos);
                }
            }
        }
    }
    ...
    pub fn render_grid(&self, ui: &mut egui::Ui, ) {
        ...
        for (i, chunk_pos) in self.0.iter().enumerate() {
            let row = i / grid_size;
            let col = i % grid_size;
