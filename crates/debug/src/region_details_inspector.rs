...
pub fn region_details_inspector(world: &mut World) {
    let selected_region_entity = world.resource::<DebugSelectedEntities>().selected_regions.iter().next().copied();

    if selected_region_entity.is_none() {
        return;
    }

    let selected_region_entity = selected_region_entity.unwrap();
    let window_visible = world.resource::<DubugWindowsVisibility>();

    if !window_visible.region_details {
        return;
    }
    ...
    egui::Window::new("Selected Region Details")
        .default_width(600.0)
        .default_height(500.0)
        .default_pos([screen_rect.right() - 620.0, screen_rect.top() + 180.0])
        .open(&mut is_open)
        .vscroll(true)
        .show(egui_context.get_mut(), |ui| {
            ...
            ui.label("All Components on this Region:");
            ui.separator();

            // Use unsafe to access world for full component inspection with values
            unsafe {
                bevy_inspector::ui_for_entity(&mut *world_ptr, selected_region_entity, ui);
            }
