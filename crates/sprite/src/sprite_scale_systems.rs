...
pub fn apply_scales(
    ...
    mut sprite_que: Query<(&mut Transform, Option<&mut Sprite>, &BaseHolderRef, &TemplEntiRef,
        Option<&FlippedTransform>, Option<&Scale2D>, Option<&ScaleLookUpDown>, Option<&ScaleSideways>,
    ),>,
    ...
    for (msg, _) in reader.par_read() {
        ...
        if let Ok((base_direction, ref_sprite_global_normal_dist_result, ref_sprite_hori_normal_dist_result, ref_sprite_vert_normal_dist_result)) = baseholder_query.get(spriteholder.base) {
            let sprite_global_mult = ref_sprite_global_normal_dist_result.map(|v| v.0).unwrap_or(1.0);
            let sprite_hori_mult = ref_sprite_hori_normal_dist_result.map(|v| v.0).unwrap_or(1.0);
            let sprite_vert_mult = ref_sprite_vert_normal_dist_result.map(|v| v.0).unwrap_or(1.0);
            total_scale *= Scale2D::from((sprite_global_mult * sprite_hori_mult, sprite_global_mult * sprite_vert_mult));
        ...
        let mut total_scale_vec2 = total_scale.as_vec2();
        if flipped_transform.is_some_and(|flipped_transform| flipped_transform.x) {
            total_scale_vec2.x = -total_scale_vec2.x.abs();
        }
        if flipped_transform.is_some_and(|flipped_transform| flipped_transform.y) {
            total_scale_vec2.y = -total_scale_vec2.y.abs();
        }
        if total_scale_vec2.x == 0.0 || total_scale_vec2.y == 0.0 {
            warn!("total_scale is zero for sprite entity");
        }
        if total_scale_vec2 != transform.scale.xy() {
