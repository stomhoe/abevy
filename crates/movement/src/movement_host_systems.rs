...
pub fn receive_step_request_from_client(
    ...
    for from_client in events.read() {
        ...
        pending_requests
            .entry(being_ent)
            .or_default()
            .push_back(DeferredStepRequest {
                client_id: from_client.client_id,
                step_dir,
                steps,
            });
    }
    let mut next_deferred_requests: EntityHashMap<VecDeque<DeferredStepRequest>> = EntityHashMap::default();
    for (being_ent, mut request_queue) in pending_requests {
        while let Some(deferred_request) = request_queue.pop_front() {
            ...
            if state.step_credit + settings.step_early_tolerance < requested_steps as f32 {
                if elapsed <= f32::EPSILON {
                    ...
                    let queue = next_deferred_requests.entry(being_ent).or_default();
                    queue.push_back(deferred_request);
                    queue.append(&mut request_queue);
                    break;
                }
                warn!(
                    target: MOVEMENT_SYSTEM,
