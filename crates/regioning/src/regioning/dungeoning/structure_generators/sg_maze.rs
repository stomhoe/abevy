...
pub fn maze_dungeon_building_system(
    ...
    for build_order in reader.read() {
        ...
        let floor_entity = match templs_map.0.get_cloned(floor_tile_id) {
            ...
            Err(_) => {
                error!(target: "dungeoning", "TileTempl '{:?}' not found for maze dungeon", floor_tile_id);
                continue;
            }
        };
        ...
        let wall_entity = match templs_map.0.get_cloned(wall_tile_id) {
            ...
            Err(_) => {
                error!(target: "dungeoning", "TileTempl '{:?}' not found for maze dungeon", wall_tile_id);
                continue;
            }
        };
        ...
        let lava_entity = match templs_map.0.get_cloned(lava_tile_id) {
            ...
            Err(_) => {
                error!(target: "dungeoning", "TileTempl '{:?}' not found for maze dungeon", lava_tile_id);
                continue;
            }
        };
        ...
        let mut chunk_tiles = Vec::with_capacity(chunk_positions.len());
