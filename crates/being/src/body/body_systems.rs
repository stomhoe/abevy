...
pub fn update_body_health_from_parts(
    ...
    >,
    mut damage_query: Query<(Entity, Option<&mut BodyPartDamage>), With<BodyPart>>,
    mut body_health_query: Query<Option<&mut BodyHealth>>,
    mut dead_query: Query<Option<&mut BodyDead>>,
    ...
    for body in bodies.iter() {
        ...
        let blood_capacity = (agg.blood_capacity
            + blood_capacity_mod_sum.get(body).copied().unwrap_or(0.0))
        .max(0.0);
        ...
        match body_health_query.get_mut(*body) {
            ...
            Ok(None) => {
                cmd.entity(*body).try_insert(BodyHealth {
        ...
        match dead_query.get_mut(*body) {
            Ok(Some(_)) if !dead => {
                cmd.entity(*body).try_remove::<BodyDead>();
            }
            Ok(None) if dead => {
                cmd.entity(*body).try_insert(BodyDead);
            }
            Ok(_) => {}
            Err(_) => {}
        }
...
pub fn apply_pain_slowdown(
    ...
    for (body_ent, body_health) in body_health_query.iter_mut() {
        ...
        // Spawn entities with only speed type components
