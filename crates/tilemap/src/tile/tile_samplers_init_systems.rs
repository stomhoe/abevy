...
use crate::tile::{tile_components::*, tile_resources::*, tile_samplers_resources::*};
    ...
    for handle in seris_handles.handles.iter() {
        if let Some(seri) = assets.get(handle) {
            ...
            if let Ok(str_id) = StrId::new(seri.id.clone(), 4) {
                cmd.spawn((str_id, HashPosEntiWeightedSampler::default(), ));
            }
        }
    }
} 
...
pub fn init_tile_weighted_samplers_weights(
    ...
    hashpos_query: Query<(&StrId, ), (With<HashPosEntiWeightedSampler>)>,
    ...
    for handle in seris_handles.handles.drain(..) {
        ...
        cmd.entity(wmap_ent).insert(HashPosEntiWeightedSampler::new(&weights));
