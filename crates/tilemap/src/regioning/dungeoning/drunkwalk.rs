...
pub fn drunkwalk_dungeon_building_system(
    ...
    for build_order in reader.read() {
        ...
        let corridor_wiggle_chance = parse_arg::<f32>(&structured_gen_cfg.args, "corridor_wiggle_chance", 0.35)
            .clamp(0.0, 1.0);
        let corridor_wiggle_step_max = parse_arg::<i32>(&structured_gen_cfg.args, "corridor_wiggle_step_max", 1)
            .clamp(0, 4);
        let corridor_detour_chance = parse_arg::<f32>(&structured_gen_cfg.args, "corridor_detour_chance", 0.0)
            .clamp(0.0, 1.0);
