...
pub fn track_spawned_tiles_for_ai_nav(
    query: Query<
        ...
        (
            Or<(Added<Tile>, Changed<GlobalTilePos>)>,
            With<Tile>,
            Without<Templ>,
            common::AnyDisabling,
        ),
    ...
    for (&dim, &gpos, tile_ref, templ_enti_ref) in &query {
        ...
        if !ai_nav_tile_blocked_gpos_counts.insert_blocked_positions(dim, gpos, interaction_zones, is_low_speed) {
            continue;
        }
        tracked_tiles += 1;
        nav_grid_dirty_msgs.insert(AiNavGridDirtyDim { dim });
    }
