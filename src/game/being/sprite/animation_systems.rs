...
use crate::{common::common_components::{DisplayName, StrId}, game::{being::{being_components::{ControlledBy, HeldSprites}, movement::movement_components::*, sprite::{animation_components::*, animation_resources::*, sprite_components::* }}, game_components::FacingDirection, player::player_components::{OfSelf, Player},}};
...
pub fn animate_sprite(
    ...
    for (ent, spriteholder_ref, sprite_cfg_ref, sheet_anim, moving_anim, ) in query.iter_mut() {
        ...
        let direction_str = if directionable {
            direction
                .ok()
                .map(|(facing_direction,)| facing_direction.as_suffix())
                .unwrap_or("")
        } 
        else {""};
