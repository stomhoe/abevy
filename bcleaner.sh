...
if [ -d "target/debug" ]; then
    find target/debug -type f -regextype posix-extended -regex ".*/${PATTERN}.*" -exec rm -f {} +
fi

# Remove directories in target/debug/incremental
INC_PATH="target/debug/incremental"
if [ -d "$INC_PATH" ]; then
    find "$INC_PATH" -mindepth 1 -maxdepth 1 -type d -regextype posix-extended -regex ".*/${PATTERN}.*" -exec rm -rf {} +
fi