    ...
    lights
        .par_splat_map_mut(ComputeTaskPool::get(), None, |_, lights| {
            ...
            for (entity, light, light_pointer, light_index, bins) in lights {
                ...
                let mut camera_views = Vec::new();

                if light.light_kind == 1 {
                    let ray_dir = light.dir.normalize_or_zero();
                    if ray_dir.length_squared() <= f32::EPSILON {
                        continue;
                    }

                    let perp = vec2(-ray_dir.y, ray_dir.x);
                    ...
                    let cameras = cameras
                        ...
                        .filter_map(|camera| {
                            ...
                            let camera_corner0 = camera_rect.min;
                            let camera_corner1 = vec2(camera_rect.min.x, camera_rect.max.y);
                            let camera_corner2 = vec2(camera_rect.max.x, camera_rect.min.y);
                            let camera_corner3 = camera_rect.max;

                            let strip_min = camera_corner0
                                .dot(perp)
                                .min(camera_corner1.dot(perp))
                                .min(camera_corner2.dot(perp))
                                .min(camera_corner3.dot(perp));
                            let strip_max = camera_corner0
                                .dot(perp)
                                .max(camera_corner1.dot(perp))
                                .max(camera_corner2.dot(perp))
                                .max(camera_corner3.dot(perp));
                            ...
                            Some((camera, camera_rect, strip_min, strip_max))
                        })
                        .collect::<Vec<_>>();

                    camera_views.extend(cameras.iter().map(|(camera, _, _, _)| camera));

                    let mut directional_occluder_hits = 0usize;
                    ...
                    for (occluder, round_index, poly_index) in &occluders {
                        ...
                        let Some(pointer_index) = (if matches!(occluder.shape, Occluder2dShape::RoundRectangle { .. }) {
                            round_index.0.map(|idx| idx.index as u32)
                        } else {
                            poly_index.occluder.map(|idx| (1 << 31) | idx.index as u32)
                        }) else {
                            continue;
                        };

                        for (camera, camera_rect, strip_min, strip_max) in &cameras {
                            if !camera.1.intersects(&occluder.render_layers) {
                                continue;
                            }

                            let diag = (camera_rect.max - camera_rect.min).length();
                            let upstream = -ray_dir * diag * 2.0;
                            let extra = Vec2::splat(diag);
                            let expanded = Aabb2d {
                                min: camera_rect.min.min(camera_rect.min + upstream) - extra,
                                max: camera_rect.max.max(camera_rect.max + upstream) + extra,
                            };

                            if !occluder.aabb.intersects(&expanded) {
                                continue;
                            }

                            directional_occluder_hits += 1;

                            let corner0 = vec2(occluder.aabb.min.x, occluder.aabb.min.y);
                            let corner1 = vec2(occluder.aabb.min.x, occluder.aabb.max.y);
                            let corner2 = vec2(occluder.aabb.max.x, occluder.aabb.min.y);
                            let corner3 = vec2(occluder.aabb.max.x, occluder.aabb.max.y);

                            let proj0 = corner0.dot(perp);
                            let proj1 = corner1.dot(perp);
                            let proj2 = corner2.dot(perp);
                            let proj3 = corner3.dot(perp);

                            let proj_min = proj0.min(proj1).min(proj2).min(proj3);
                            let proj_max = proj0.max(proj1).max(proj2).max(proj3);

                            let ray0 = corner0.dot(ray_dir);
                            let ray1 = corner1.dot(ray_dir);
                            let ray2 = corner2.dot(ray_dir);
                            let ray3 = corner3.dot(ray_dir);
                            let ray_min = ray0.min(ray1).min(ray2).min(ray3);

                            let span = (*strip_max - *strip_min).max(1e-6);
                            let min_bin = (((proj_min - *strip_min) / span)
                                .clamp(0.0, 1.0)
                                * ((crate::buffers::N_BINS - 1) as f32))
                                .floor() as usize;
                            let max_bin = (((proj_max - *strip_min) / span)
                                .clamp(0.0, 1.0)
                                * ((crate::buffers::N_BINS - 1) as f32))
                                .ceil() as usize;
                    ...
                    trace!(target: "lighting_init", "Prepared directional light bins light_entity={:?} cameras={} occluder_hits={} ray_dir={:?}", entity, camera_views.len(), directional_occluder_hits, ray_dir);
                } else {
                    let cameras = cameras
                        .iter()
                        .filter_map(|camera| {
                    ...
                    camera_views.extend(cameras.iter().map(|(camera, _)| *camera));
                ...
                for camera in camera_views.iter() {
                    let bins = bins.0.get_mut(&camera.0.retained_view_entity).unwrap();
