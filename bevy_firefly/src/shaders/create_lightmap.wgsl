...
fn fragment(in: FullscreenVertexOutput) -> @location(0) vec4f {
    ...
    var ray_dir = -light.dir;
    if dot(ray_dir, ray_dir) < 0.000001 {
        return vec4f(0.0);
    }
    ...
    if config.normal_mode != 0 && normal.a > 0 && normal.b != 0.1 {
        ...
        let light_dir = normalize(vec3f(-ray_dir.x, -ray_dir.y, 1.0));
    ...
    res = vec4f(light_color.xyz, 0) * light.intensity * normal_multi;
    ...
    let perp = normalize(vec2f(-ray_dir.y, ray_dir.x));
    let view_corner0 = ndc_to_world(vec2f(-1.0, -1.0));
    let view_corner1 = ndc_to_world(vec2f(-1.0, 1.0));
    let view_corner2 = ndc_to_world(vec2f(1.0, -1.0));
    let view_corner3 = ndc_to_world(vec2f(1.0, 1.0));

    let strip_min = min(
        min(dot(view_corner0, perp), dot(view_corner1, perp)),
        min(dot(view_corner2, perp), dot(view_corner3, perp)),
    );
    let strip_max = max(
        max(dot(view_corner0, perp), dot(view_corner1, perp)),
        max(dot(view_corner2, perp), dot(view_corner3, perp)),
    );

    let coord = dot(pos, perp);
    let bin_factor = clamp((coord - strip_min) / max(strip_max - strip_min, 0.000001), 0.0, 1.0);
    let bin = u32(floor(bin_factor * (f32(N_BINS) - 1.0)));
    let ray_coord = dot(pos, ray_dir);

    let left = bin_indices.indices[bin];
    let right = bin_indices.indices[bin + 1u];

    for (var pointer_index = left; pointer_index < right; pointer_index += 1) {
        let pointer = occluders[pointer_index];
        if pointer.distance > ray_coord {
            break;
        }
        ...
        if occluder_type == 0 {
            ...
            let result = directional_round_check(pos, ray_dir, occluder_index);
            if result > 0.0 {
            ...
            let result = directional_poly_check(pos, ray_dir, occluder_index);
            if result > 0.0 {
...
fn directional_poly_check(pos: vec2f, ray_dir: vec2f, index: u32) -> f32 {
    let occluder = poly_occluders[index];
    if occluder.n_vertices < 2u {
        return 0.0;
    }

    let ray_target = pos + ray_dir;

    for (var i = 0u; i + 1u < occluder.n_vertices; i += 1u) {
        let v1 = vertices[occluder.start_vertex + i];
        let v2 = vertices[occluder.start_vertex + i + 1u];
        if intersects_half(pos, ray_target, v1, v2) {
            return 1.0;
        }
    }

    if occluder.is_polyline == 0u {
        let last = vertices[occluder.start_vertex + occluder.n_vertices - 1u];
        let first = vertices[occluder.start_vertex];
        if intersects_half(pos, ray_target, last, first) {
            return 1.0;
        }
    }
...
fn directional_round_check(pos: vec2f, ray_dir_world: vec2f, occluder: u32) -> f32 {
    ...
    var ray_local = vec2f(ray_dir_world.x * c + ray_dir_world.y * s, -ray_dir_world.x * s + ray_dir_world.y * c);
    if dot(ray_local, ray_local) < 0.000001 {
        return 0.0;
    }
    ray_local = normalize(ray_local);
    let ray_target = p_local + ray_local;

    let closest = vec2f(clamp(p_local.x, -half_w, half_w), clamp(p_local.y, -half_h, half_h));
    if distance(p_local, closest) <= radius {
        return 1.0;
    }

    if half_w > 0.0 {
        let top_a = vec2f(-half_w, half_h + radius);
        let top_b = vec2f(half_w, half_h + radius);
        if intersects_half(p_local, ray_target, top_a, top_b) {
            return 1.0;
        }

        let bottom_a = vec2f(-half_w, -(half_h + radius));
        let bottom_b = vec2f(half_w, -(half_h + radius));
        if intersects_half(p_local, ray_target, bottom_a, bottom_b) {
            return 1.0;
        }
    }

    if half_h > 0.0 {
        let right_a = vec2f(half_w + radius, -half_h);
        let right_b = vec2f(half_w + radius, half_h);
        if intersects_half(p_local, ray_target, right_a, right_b) {
            return 1.0;
        }

        let left_a = vec2f(-(half_w + radius), -half_h);
        let left_b = vec2f(-(half_w + radius), half_h);
        if intersects_half(p_local, ray_target, left_a, left_b) {
            return 1.0;
        }
    }

    if radius > 0.0 {
        if intersects_corner_arc_half(p_local, ray_local, vec2f(half_w, half_h), radius, vec2f(1,1)) {
            return 1.0;
        }
        if intersects_corner_arc_half(p_local, ray_local, vec2f(-half_w, half_h), radius, vec2f(-1,1)) {
            return 1.0;
...
fn intersects_corner_arc_half(origin: vec2f, dir: vec2f, center: vec2f, radius: f32, quadrant: vec2f) -> bool {
    let f = origin - center;
    let a = dot(dir, dir);
    let b = 2.0 * dot(f, dir);
    ...
    if t1 >= 0.0 {
        let hit1 = origin + dir * t1 - center;
        if sign(hit1.x) == quadrant.x && sign(hit1.y) == quadrant.y {
            return true;
        }
    }

    if t2 >= 0.0 {
        let hit2 = origin + dir * t2 - center;
        if sign(hit2.x) == quadrant.x && sign(hit2.y) == quadrant.y {
            return true;
        }
    }
