...
use crate::{
    ...
    lights::{DirectionalLight2d, ExtractedPointLight, LightHeight, PointLight2d},
...
fn extract_directional_lights(
    ...
    const DIRECTIONAL_BIN_HALF_SPAN: f32 = 65536.0;

    for (entity, light, visibility, visibility_timer, changes, render_layers) in &lights {
        if !visibility.get() {
            if visibility_timer.0.just_finished() {
                commands.entity(entity).insert(NotVisible);
            }
        ...
        let mut dir_xy = light.direction.xy();
        if dir_xy.length_squared() <= f32::EPSILON {
            dir_xy = Vec2::NEG_Y;
        }

        trace!(target: "lighting_init", "Extract directional light entity={:?} dir={:?} intensity={:.3}", entity, dir_xy.normalize_or_zero(), light.intensity);
        commands.entity(entity).insert(ExtractedPointLight {
            light_kind: 1,
            pos: Vec2::ZERO,
            color: light.color,
            ...
            radius: 0.0,
            z: DIRECTIONAL_BIN_HALF_SPAN,
            core: default(),
            falloff: crate::lights::Falloff::None,
            angle: crate::lights::LightAngle::FULL,
            cast_shadows: true,
            dir: dir_xy.normalize_or_zero(),
            height: 1.0 / (2.0 * DIRECTIONAL_BIN_HALF_SPAN),
            changes: changes.clone(),
            render_layers: render_layers.clone(),
        });
    }
